...
- **네이버 라우트엔 대표 사진 폴백이 아예 없어서, `naverPlaceId`가 등록된 식당은 카카오 쪽에 사진이 있어도 절대 못 가져오는 문제** (2026-09-22, 위 항목과 같은 날 발견됐지만 원인은 다름): "여장군 가산점"(오버라이드 없는 순수 카카오 라이브 데이터)은 위 colo 캐시 문제였지만, "텐진라멘 가산디지털단지점"은 공유 오버라이드(`/overrides`)에 `naverPlaceId:"1503592037"`가 등록돼 있어 `enrichMenus`가 네이버 메뉴를 먼저 시도하고 성공하면 카카오는 아예 호출하지 않는 구조라, 대표사진 추출 로직이 카카오(panel3) 라우트에만 있고 네이버 라우트(`handleNaverMenu`)엔 없었던 게 그대로 드러남(naverPlaceId 등록 식당 전체가 영향받는 구조적 누락, colo 캐시처럼 시간 지나면 저절로 풀리는 문제가 아님). 네이버 페이지를 직접 받아 `__APOLLO_STATE__`를 까본 결과 `PlaceDetailTopPhotoItem` typename에 상단 사진탭 데이터가 있고, `mediaSource:'business'`가 사장님이 직접 등록한 사진(리뷰 사진보다 신뢰도 높음)이라 이를 최우선, 없으면 방문자 리뷰 사진(`aiView`/`placeReview`) 첫 장으로 폴백하도록 `handleNaverMenu`에 `representativePhotoUrl` 계산을 추가함. 응답 모양이 바뀌므로 `NAVER_MENU_SCHEMA_VERSION`도 v2→v3로 같이 올림.
