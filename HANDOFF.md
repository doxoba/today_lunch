...
- **구내식당/한식뷔페 "오늘의 메뉴" 사진** (2026-09-17 추가): 많은 구내식당은 메뉴를 텍스트가 아니라 카카오톡 채널 프로필 사진으로 올린다. `pf.kakao.com/rocket-web/web/v2/profiles/{채널ID}`가 로그인 쿠키 없이도 프로필 이미지 URL(`profile_image.xlarge_url`)을 내려주는 사실상 공개 엔드포인트임을 직접 curl로 검증하고(개인 세션 쿠키 불필요 — 만료/계정 플래그 리스크 없음), `kakao-menu-proxy.worker.js`에 `?cafeteriaChannel={채널ID}` 라우트로 추가했다. 식당↔채널ID 매핑은 네이버 place id와 동일하게 "정보 편집" 패널에서 수동 입력(`cafeteriaChannelId` override)한다. Worker의 `scheduled()`가 매일 09:00(KST)에 `CAFETERIA_CHANNELS` 목록을 미리 캐시에 데워두지만, **이 cron 자체는 Cloudflare 대시보드(Workers & Pages > 해당 Worker > Settings > Triggers > Cron Triggers)에 `0 0 * * *`를 직접 등록해야 동작한다** (코드만으로는 cron 등록이 안 됨 — 아직 미등록 상태). 카카오 채널 프로필 카드의 `updated_at`이 "오늘 갱신"인지 판단하는 유일한 신호라 프론트엔드가 그 날짜를 그대로 보여주고(오늘이 아니면 경고 문구), 실제로 매일 사진을 갱신하는 채널인지는 가게마다 다르므로 사용자가 눈으로 확인해야 한다.
...
- [ ] **Cloudflare 대시보드에서 Worker(`kakao-menu-proxy`)에 Cron Trigger `0 0 * * *`(UTC 0시=KST 9시) 등록** — 구내식당 메뉴 사진을 매일 아침 미리 캐시해두는 기능인데, 코드만 배포해선 동작 안 하고 대시보드에서 직접 등록해야 함 (Workers & Pages > 해당 Worker > Settings > Triggers > Cron Triggers)
- [ ] 씽씽푸드/더소울푸드/바른식탁 외에 구내식당/한식뷔페로 분류된 식당들도 "정보 편집" 패널에서 카카오톡 채널ID를 찾아 입력해주기 (자동 매칭 불가 — 수동 입력만 지원)
