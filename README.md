...
- **메뉴 검색**: "알밥" 같은 구체적인 메뉴 이름(또는 상호명)으로 반경 내 식당을 바로 찾음. 검색 중에는 형태/종류/목록 카테고리 필터를 전부 무시하고 반경 내 전체 식당에서 찾고, 결과가 0건이면 "500m까지 넓혀서 다시 찾기" 버튼으로 반경을 넓혀 재검색할 수 있음(실제 메뉴를 아직 못 가져온 식당은 검색에 안 걸릴 수 있음)
...
- **수동 큐레이션, 전부 공유됨**: 종류/실제 메뉴/네이버 place id/카카오톡 채널ID를 직접 덮어쓰거나, 폐업 등으로 카카오에 계속 남아있는 상점을 포함/제외로 보정할 수 있고, 한 번 입력하면 모든 사용자·모든 기기에 그대로 보임
- **사진 리뷰**: 식당마다 사진+텍스트로 짧은 리뷰(댓글)를 남기고 볼 수 있음
- **"🎇우리 오늘 뭐먹지" 팀 모드**: 초대 코드로 방을 만들어 팀원들이 각자 조건(제외 항목, 반경)을 제출하면 다같이 갈 만한 곳을 함께 추천받음
- **설정 저장**: 식당별 편집 내용(종류/메뉴/채널ID/포함·제외 등)과 리뷰, 팀 모드 방 상태는 Cloudflare Worker/KV에 공유 저장되고, 반경 선택처럼 순수 개인 설정만 브라우저 로컬 스토리지에 남음
...
- 사용자가 덮어쓴 종류/메뉴/네이버 place id/카카오톡 채널ID는 식당 ID 기준으로 Cloudflare KV(공유 저장소)에 저장되어 다른 사용자·다른 기기에서도 그대로 보임(2026-09-21 이전엔 이 기기에만 저장돼 다른 사람은 못 봤음 — 아래 "팀 모드 & 공유 상태" 참고). `localStorage`는 오프라인 폴백/이전 버전 호환용으로 계속 같이 씀
...
- 식당별 "정보 편집" 내용(자동/포함/제외, 종류, 실제 메뉴, 네이버 place id, 카카오톡 채널ID, 구내식당 수동/주간 사진 등록 여부)은 모두 Cloudflare KV에 영구 공유됨 — 한 사람이 입력하면 다른 사용자·다른 기기에도 그대로 보임. 로그인이 없는 앱이라 작성자 구분 없이 누구나 다시 고칠 수 있음(폐업 등으로 카카오에 계속 남아있는 상점을 한 사람이 "제외" 처리하면 모두에게 공유되는 것과 같은 원칙)
- 기기에 남아있던 이전 로컬 편집 내용은 처음 접속할 때 공유 저장소로 한 번만 자동 업로드됨(그 이후엔 공유 저장소가 항상 우선)
...
- 종류, 추천 메뉴, 다른 메뉴 항목 편집 — **저장하면 모든 사용자·기기에 공유됨**
- 경계 보정을 위한 자동/포함/제외 토글 — **공유됨**(폐업 등은 한 번만 처리하면 됨)
- 네이버 place id, 카카오톡 채널ID 등록(실제 메뉴/오늘의 메뉴 사진 자동 조회용) — **공유됨**
- 구내식당 오늘의 메뉴 사진 직접 붙여넣기, 요일별 주간 식단표 5칸 등록 및 등록 여부 확인
- "편집 내용 초기화"로 되돌리면 로컬과 공유 저장소 양쪽에서 함께 지워짐
- 위 항목은 모두 Worker/KV에 공유 저장되고(로그인이 없어 작성자 구분 없이 누구나 다시 고칠 수 있음), `localStorage`는 오프라인 폴백/이전 버전 호환용으로만 같이 씀 — 이 기기에 예전부터 있던 편집 내용은 처음 접속 시 공유 저장소로 한 번 자동 업로드됨
...
"주변 식당 목록" 패널 상단 검색창에 메뉴 이름(또는 상호명)을 입력하면 즉시 그 이름이 포함된 식당만 걸러서 보여줍니다.

- **검색 범위**: 현재 선택된 반경 안에서만 찾습니다. 결과가 0건이면 안내 문구와 함께 "500m까지 넓혀서 다시 찾기" 버튼이 나타나고, 누르면 반경을 500m로 바꿔 재검색한 뒤 같은 검색어로 자동 재적용됩니다.
- **필터와의 관계**: 검색 중에는 형태/종류 칩과 목록 카테고리 필터를 모두 무시합니다 — 필터가 켜져 있어서 정작 찾는 식당이 숨어버리는 일을 막기 위함입니다.
- **한계**: 검색은 각 식당의 실제 메뉴 데이터(카카오/네이버에서 가져온 `menuItems`)와 상호명만 대상으로 합니다. 실제 메뉴를 아직 못 가져온 식당은 일반 명사(예: "돈까스", "국밥") 하나짜리 추정 메뉴만 갖고 있어서, "알밥"처럼 구체적인 메뉴명으로는 걸리지 않을 수 있습니다.
...
6. **메뉴 검색의 커버리지는 실제 메뉴 확보율에 좌우됨**: 검색은 카카오/네이버에서 가져온 실제 메뉴명과 상호명만 대상으로 합니다. 아직 실제 메뉴를 못 가져온 식당은 일반 명사 추정 메뉴 하나만 갖고 있어서(`TAG_RULES`에 등록된 단어에만 매칭), "알밥"처럼 구체적인 메뉴명으로는 실제로 그 메뉴를 파는데도 검색에 안 걸릴 수 있습니다.
   - 완화책: 반경을 넓혀보거나(검색 결과 0건일 때 뜨는 버튼), 상호명으로도 검색해보세요
